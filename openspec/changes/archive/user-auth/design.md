...
- `database-bootstrap-commands`、`project-setup`和`framework-bootstrap-installer`受`make init`/`make mock`写入 PLATFORM、开发入口、多租户启动校验和 PostgreSQL-only 初始化影响；当前契约由`archive/database-engine`、`archive/foundation`或开发工具主规范承载。
