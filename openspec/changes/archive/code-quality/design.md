...
`lina-review`纳入 API DTO 响应实体暴露、时间字段、后端主文件职责、接口注释、接口方法唯一性、显式依赖注入、缓存敏感实例来源、初始化错误返回、测试效率和归档治理影响判断。历史任务通过 Go 单元测试、E2E、静态扫描、OpenSpec 校验、`make init`、`make dao`、`make ctrl`、前端类型和审查闭环验证。
