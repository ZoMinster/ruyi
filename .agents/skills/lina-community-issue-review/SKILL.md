...
9. 描述模糊、无法判断、骚扰或广告类`Issue`必须评论说明原因或补充要求，然后关闭。
...
- 可行时评论评估结果，添加`feature`标签，保持`Issue`开放等待实现。
...
- 可行且需要修复时评论评估结果，添加`bug`标签，保持`Issue`开放等待修复。
...
1. 评论说明关闭原因；如果只是模糊，说明需要补充的最小信息。
2. 发布带`status=invalid`隐藏标记的评论。
3. 关闭`Issue`。
