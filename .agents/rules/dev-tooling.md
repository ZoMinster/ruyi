...
- 新增或修改`.sh`、`.ps1`、`.cmd`、`.mjs`、Node 脚本或工具型 Go 代码
...
- 推荐验证包括`cd hack/tools/linactl && go test ./... -count=1`、`go run ./hack/tools/linactl test.scripts`、目标 Go 工具单元测试或跨平台静态扫描。
- 仅使用`bash -n`、本机 Shell 试跑或单平台 workflow 不能证明跨平台合规。
- 若本次变更确认不影响开发工具或脚本，应在任务记录或审查结论中明确说明。

## 审查要求

- 审查必须确认默认开发路径没有新增平台专属命令依赖。
- 审查必须确认长期维护逻辑没有继续沉淀在 Shell 管道或平台脚本中。
- 审查必须确认`linactl`命令文件命名和内部组件组织合规。
- 审查必须确认跨平台验证证据匹配变更范围。
